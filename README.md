...
- log the quiz results to `LEARNING.md`, so the next session skips what you've got and
  revisits what you haven't
...
The skill loads when its description matches what you're doing, which is fine for a session
where you've said you're learning. For a project where learning mode should always be on, name
the skill in your `CLAUDE.md` — that file loads every session, so it triggers reliably:
...
Concepts get promoted to "mastered" after two correct answers, not one. Answering right about
something you were walked through five minutes ago mostly proves you were paying attention.
